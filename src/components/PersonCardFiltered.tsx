...
interface PersonCardFilteredProps {
  person: Person;
...
export function PersonCardFiltered({ person, excludeStatus, onUpdatePerson, onDeletePerson }: PersonCardFilteredProps) {
  const [isEditing, setIsEditing] = useState(false);
  const [editName, setEditName] = useState(person.name);
  const [newTaskTitle, setNewTaskTitle] = useState('');
  const [isAddingTask, setIsAddingTask] = useState(false);

  // Filter out tasks with the excluded status
  const filteredTasks = person.tasks.filter(task => task.status !== excludeStatus);
    ...
    <Card className="w-full">
      <CardHeader className="pb-3">
        ...
        <div className="text-sm text-muted-foreground">
          {sortedTasks.length}/{person.tasks.length} tareas visibles
        </div>
      </CardHeader>
      <CardContent className="space-y-3">
        ...
        {sortedTasks.length === 0 && (
          <div className="text-center py-4 text-gray-500 text-sm">
            Todas las tareas están en estado "{TASK_STATUS_LABELS[excludeStatus]}"
          </div>
        )}
